...
(()=>{
  ...
  const vehicleTune={acceleration:1,topSpeed:120};
  document.addEventListener('jd:vehicle',e=>{
    ...
    vehicleTune.acceleration=Math.max(1,Math.min(4,acceleration));
    vehicleTune.topSpeed=Math.max(120,Math.min(300,topSpeed));
  ...
  loop=t=>{const beforeSpeed=state.speed;drivetrainLoop(t);if(state.gear===0)state.peakLaunchRpm=Math.max(state.peakLaunchRpm||900,launchRpm);if(state.phase!=='GO'&&state.gear!==0){launchRpm=900;state.rpm=900;displayRpm=900;rpm.textContent='900'}if(state.phase==='GO'&&state.speed>beforeSpeed){state.speed+=Math.min(0.35,(state.speed-beforeSpeed)*0.46)}};
  ...
  finish=()=>{
    ...
    document.dispatchEvent(new CustomEvent('jd:race-finished',{detail:{win:state.dist>=state.cpu,perfect:state.perfect,launch:state.launch}}));
    renderRoad();
    feedback('FINISH!');
